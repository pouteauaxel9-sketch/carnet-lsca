(function () {
  ...
  const COLORS = ['#009640', '#0f6e56', '#854f0b', '#993556', '#5f5e5a', '#d85a30'];

  function loadEducator() {
    try {
      return JSON.parse(localStorage.getItem(EDUCATOR_KEY) || 'null') ||
        { id: 'local', name: '', color: '#009640' };
    } catch { return { id: 'local', name: '', color: '#009640' }; }
  }
  ...
  function renderBadgeHtml() {
    ...
    return `<button class="educator-badge" type="button" data-educator-action="open-config" title="Profil éducateur" style="${ed.name ? 'border-color:' + ed.color : ''}">
      <span class="educator-dot" style="background:${ed.color || '#009640'}"></span>
      <span>${h(name)}</span>
    </button>`;
  }
    ...
    return `<div class="modal-overlay" id="educator-modal-overlay">
      <div class="modal-box" style="max-width:400px">
        <div class="modal-head">
          <div><div class="card-kicker">Profil</div><h3>Éducateur</h3></div>
        ...
        <div class="modal-footer">
          <button class="btn btn-ghost" type="button" data-educator-action="close-config">Annuler</button>
          <button class="btn btn-primary" type="button" data-educator-action="save-config">Enregistrer</button>
        </div>
      </div>
    </div>`;
  }
  ...
  function handleAction(target) {
    ...
    if (ea === 'open-config') {
      configOpen = true;
      tempColor = loadEducator().color || '#009640';
      renderModal();
      return true;
    }
    if (ea === 'close-config') {
    ...
    if (ea === 'save-config') {
      ...
      const ed = loadEducator();
      const color = tempColor || ed.color || '#009640';
  ...
  window.EducatorModule = {
    getEducator, getEducatorName, getEducatorId,
    injectBadge, handleAction, renderModal
  };
